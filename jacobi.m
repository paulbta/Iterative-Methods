function [x, residual, numItr] = jacobi(A, b, x0, ConvCrit)

    % Additional inputs:
   
    rlx=1; % Relaxation factor
    n=length(x0);
    x = x0; xold = x0;
    numItr=0;
    residual=1;
    ...
    while residual >= ConvCrit % Continue until the residual is smaler than the convergence criteria
        ...
        for i=1:n % Loop over gridpoints
            ...
            x(i) = (1-rlx)*xold(i) + rlx*(-A(i,I_lhs)*xold(I_lhs)/A(i,i) - A(i,I_rhs)*xold(I_rhs)/A(i,i) +  b(i)/A(i,i));
        end
